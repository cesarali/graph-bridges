...
from graph_bridges.data.ising_dataloaders import ParametrizedIsingHamiltonianLoader
...
def load_dataloader(config,type:str="data",device:torch.device=torch.device("cpu"),rank=None):
    if type == "data":
        ...
        elif config.data.data in ["spin_glass","ising"]:
            dataloader = ParametrizedIsingHamiltonianLoader(config.data,device)
        else:
        ...
        if config.target.data in ['grid','community','community_small',"ego_small",'ENZYMES','QM9','ZINC250k']:
            ...
            dataloader = DoucetTargetData(config,device)
        elif config.target.data in ["spin_glass","ising"]:
            dataloader = ParametrizedIsingHamiltonianLoader(config.target,device)
        else:
            raise Exception("{0} not found in dataloaders".format(config.data.data))
    return dataloader