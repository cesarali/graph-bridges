...
from graph_bridges.models.metrics.evaluation.mmd import process_tensor, compute_mmd, gaussian, gaussian_emd

PRINT_TIME = True

"""
g++ -O2 -std=c++11 -o orca orca.cpp -static-libstdc++ -static-libgcc
"""
#C:\Users\cesar\Desktop\Projects\DiffusiveGenerativeModelling\Codes\discrete-diffusion\src\discrete_diffusion\models\graphs\evaluation\orca
from graph_bridges import project_path
project_path = Path(project_path)
ORCA_DIR = project_path / "src" / "graph_bridges" / "models" / "metrics" / "evaluation" / "orca"
...
COUNT_START_STR_DD = 'orbit counts: \r\n'
...
def orca(graph,windows=True):
    ...
    if windows:
        ...
        split_string = '\n'
    idx = output.find(COUNT_START_STR_) + len(COUNT_START_STR_)
    output = output[idx:]
    node_orbit_counts = np.array([list(map(int, node_cnts.strip().split(' ')))
                                  for node_cnts in output.strip(split_string).split(split_string)])
