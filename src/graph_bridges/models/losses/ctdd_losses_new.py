...
class GenericAux():
    ...
    def calc_loss(self, minibatch,model,x_logits,p0t_reg,p0t_sig,reg_x, ts, n_iter,device):
        """
        ...
        x_t, x_tilde,qt0,rate = self.add_noise(minibatch, model,ts)

        x_logits,p0t_reg,p0t_sig,reg_x

        p0t_reg,reg_term = self.first_term_of_elbo(minibatch,device,p0t_reg,reg_x,qt0,rate)

        outer_sum_sig,x_tilde_mask = self.second_term_of_elbo(minibatch,device,
                                                              x_tilde,qt0,rate,p0t_sig)

        loss, sig_mean, reg_mean = self.second_term_of_normalization(minibatch, device,
                                                                     x_tilde,x_tilde_mask,
                                                                     x_logits,qt0,rate,reg_term,outer_sum_sig)

        return loss

    def add_noise(self, minibatch, model,ts):
        S = self.cfg.data.S
        if len(minibatch.shape) == 4:
            B, C, H, W = minibatch.shape
            minibatch = minibatch.view(B, C * H * W)
    ...
    def first_term_of_elbo(self, minibatch,device,p0t_reg,reg_x,qt0,rate):
        S = self.cfg.data.S
        if len(minibatch.shape) == 4:
            B, C, H, W = minibatch.shape
            minibatch = minibatch.view(B, C * H * W)
    ...
    def second_term_of_normalization(self,minibatch, device,
                                     x_tilde,x_tilde_mask,
                                     x_logits,qt0,rate,reg_term,outer_sum_sig):
        """
...
if __name__=="__main__":
    ...
    from graph_bridges.dataloaders import DoucetTargetData
