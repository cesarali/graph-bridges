...
class TestGaussianReference(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.x_adj_data = next(self.data_dataloader.train().__iter__())[0]
        self.x_adj_target = next(self.data_dataloader.train().__iter__())[0]
        self.batch_size = self.x_adj_data.shape[0]
        self.times = torch.rand(self.batch_size)
